...
public class Anthill3 {
	...
	public void main(String[] args) throws IOException, CloneNotSupportedException {
		...
		Clazz tipoCliente = buildTipoCliente();
		clazzList.add(tipoCliente);		
	...
	private static Clazz buildUsuario() throws CloneNotSupportedException {
	...
	private static Clazz buildTipoCliente() throws CloneNotSupportedException {

		Clazz tipoCliente = new Clazz();
		tipoCliente.setNamePackage("clientes");
		tipoCliente.setSingular("Tipo de cliente");
		tipoCliente.setPlural("Tipos de clientes");
		tipoCliente.setSingularPre("el tipo de cliente");
		tipoCliente.setPluralPre("los tipos de clientes");
		...
		numero.setRequired(true);
		numero.setUnique(true);
		tipoCliente.addAtt(numero);

		Att nombre = new Att("nombre", "Nombre");
		nombre.setRequired(true);
		nombre.setUnique(true);
		nombre.setLength(null, 50);
		tipoCliente.addAtt(nombre);

		tipoCliente.addArgument(numero, true);
		tipoCliente.getLastArgument().setRequired(false);
		tipoCliente.addArgumentSBX(tipoCliente.getLastArgument());

		tipoCliente.addArgument(nombre);
		tipoCliente.getLastArgument().setRequired(false);
		tipoCliente.addArgumentSBX(tipoCliente.getLastArgument());

		tipoCliente.addOrderAllAtts();

		tipoCliente.getOrderDefault().setDesc(true);
	...
	private static Clazz buildZona() throws CloneNotSupportedException {

		// 'SELECT  A.ZONA, A.NOMBRE, A.BONIFICACION, A.RECARGO FROM Zonas A ORDER BY  A.ZONA'

		Clazz zona = new Clazz();
		zona.setNamePackage("geo");
		zona.setSingular("Zona");
		zona.setPlural("Zonas");
		zona.setSingularPre("la zona");
		zona.setPluralPre("las zonas");
		...
		zona.addArgument(codigo, Argument.EQUALS_IGNORE_CASE);
		zona.getLastArgument().setRequired(false);
		zona.addArgumentSBX(zona.getLastArgument());

		zona.addArgument(nombre);
		zona.getLastArgument().setRequired(false);
		zona.addArgumentSBX(zona.getLastArgument());

		zona.addOrderAllAtts();

		return zona;
	}
	
	private static Clazz buildPais() throws CloneNotSupportedException {
	...
	private static Clazz buildProvincia(Clazz paisClazz) throws CloneNotSupportedException {
	...
	private static Clazz buildCiudad(Clazz paisClazz, Clazz provinciaClazz) throws CloneNotSupportedException {
	...
	private static Clazz buildMonedaAFIP() throws CloneNotSupportedException {
...
//		monedaAFIP.addOrder(nombre);
		...
		monedaAFIP.addOrderAllAtts();

		return monedaAFIP;
	}

	private static Clazz buildMoneda(Clazz monedaAFIPClazz) throws CloneNotSupportedException {
	...
	private static Clazz buildMonedaCotizacion(Clazz monedaClazz, Clazz usuarioClazz) throws CloneNotSupportedException {
