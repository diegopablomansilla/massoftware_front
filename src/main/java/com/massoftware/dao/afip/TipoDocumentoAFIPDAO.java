...
public class TipoDocumentoAFIPDAO {
	...
	public List<TipoDocumentoAFIP> find(TipoDocumentoAFIPFiltro filtro) throws Exception {
		...
		if(orderByString != null && orderByString.trim().length() > 0) {
			...
			orderByString = "TipoDocumentoAFIP" + orderByString;
		...
		String sql = "SELECT * FROM massoftware.f_TipoDocumentoAFIP" + orderByString + levelString + "(" + params + "?, ?, ?, ?, ?, ?, ?)";
