...
public class WLTransporte extends WindowListado {
	...
	protected void addBeansToItemsBIC(boolean removeAllItems) {
		...
		try {
			...
			if (filterBI.getBean().equals(lastFilter) == false) {						
				...
				List<Transporte> items = getService().find(filterBI.getBean());
	...
	protected WindowForm buildWinddowForm(String mode, String id) throws Exception {
		return AppCX.widgets().buildWFTransporte(mode, id);
	}
...
// GENERATED BY ANTHILL 2019-06-19T12:29:54.101-03:00[America/Buenos_Aires]