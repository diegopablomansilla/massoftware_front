...
public class WLSeguridadPuerta extends WindowListado {

	// -------------------------------------------------------------

	BeanItem<SeguridadPuertaFiltro> filterBI;
	...
	public WLSeguridadPuerta() {
		super();		
		filterBI = new BeanItem<SeguridadPuertaFiltro>(new SeguridadPuertaFiltro());
		init(false);
		setFocusGrid();
	}

	public WLSeguridadPuerta(SeguridadPuertaFiltro filtro) {
		super();		
		filterBI = new BeanItem<SeguridadPuertaFiltro>(filtro);
		init(true);
		setFocusGrid();
	}
	...
	protected void buildContent() throws Exception {
		...
		confWinList(this, new SeguridadPuerta().labelPlural());
	...
	protected void buildItemsGRDLayout() throws Exception {		
		...
		itemsGRD.setColumns(new Object[] { "id", "numero", "nombre", "equate", "seguridadModulo" });
		...
		UtilUI.confColumn(itemsGRD.getColumn("equate"), true, 240);

		UtilUI.confColumn(itemsGRD.getColumn("seguridadModulo"), true, -1);
	...
	protected void addBeansToItemsBIC(boolean removeAllItems) {
		...
		try {
			...
			// List<SeguridadPuerta> items = new SeguridadPuerta().find(limit, offset, buildOrderBy(),
			// filterBI.getBean());
			...
			if (filterBI.getBean().equals(lastFilter) == false) {						
				...
				lastFilter = (SeguridadPuertaFiltro) filterBI.getBean().clone();
				...
				List<SeguridadPuerta> items = getService().find(filterBI.getBean());
	...
	protected WindowForm buildWinddowForm(String mode, String id) throws Exception {
		return AppCX.widgets().buildWFSeguridadPuerta(mode, id);
	}
...
// GENERATED BY ANTHILL 2019-06-29T09:32:08.356-03:00[America/Buenos_Aires]