...
public class WLSeguridadModulo extends WindowListado {

	// -------------------------------------------------------------

	BeanItem<SeguridadModuloFiltro> filterBI;
	protected BeanItemContainer<SeguridadModulo> itemsBIC;
	...
	public WLSeguridadModulo() {
		super();		
		filterBI = new BeanItem<SeguridadModuloFiltro>(new SeguridadModuloFiltro());
		init(false);
		setFocusGrid();
	}

	public WLSeguridadModulo(SeguridadModuloFiltro filtro) {
		super();		
		filterBI = new BeanItem<SeguridadModuloFiltro>(filtro);
		init(true);
		setFocusGrid();
	}
	
	protected SeguridadModuloService getService() throws Exception {
		if(service == null){
			service = AppCX.services().buildSeguridadModuloService();
		}
	...
	protected void buildItemsGRDLayout() throws Exception {		
		...
		SeguridadModulo dto = new SeguridadModulo();
	...
	protected BeanItemContainer<SeguridadModulo> getItemsBIC() {
		...
		if (itemsBIC == null) {
			...
			itemsBIC = new BeanItemContainer<SeguridadModulo>(SeguridadModulo.class, new ArrayList<SeguridadModulo>());
		}
	...
	protected void addBeansToItemsBIC(boolean removeAllItems) {
		...
		try {
			...
			// List<SeguridadModulo> items = new SeguridadModulo().find(limit, offset, buildOrderBy(),
			// filterBI.getBean());
			...
			if (filterBI.getBean().equals(lastFilter) == false) {						
				...
				lastFilter = (SeguridadModuloFiltro) filterBI.getBean().clone();
				...
				List<SeguridadModulo> items = getService().find(filterBI.getBean());
				
				// Agrega los resultados a la grilla
				for (SeguridadModulo item : items) {
	...
	protected WindowForm buildWinddowForm(String mode, String id) throws Exception {
		return AppCX.widgets().buildWFSeguridadModulo(mode, id);
	}
...
// GENERATED BY ANTHILL 2019-06-20T16:20:14.613-03:00[America/Buenos_Aires]