...
public class WLUsuario extends WindowListado {
	...
	protected void addBeansToItemsBIC(boolean removeAllItems) {
		...
		try {
			...
			if (filterBI.getBean().equals(lastFilter) == false) {						
				...
				List<Usuario> items = getService().find(filterBI.getBean());
	...
	protected WindowForm buildWinddowForm(String mode, String id) throws Exception {
		return AppCX.widgets().buildWFUsuario(mode, id);
	}
...
// GENERATED BY ANTHILL 2019-06-20T16:41:26.156-03:00[America/Buenos_Aires]