...
public class WLTipoCliente extends WindowListado {

	// -------------------------------------------------------------

	BeanItem<TipoClienteFiltro> filterBI;
	protected BeanItemContainer<TipoCliente> itemsBIC;
	...
	public WLTipoCliente() {
		super();		
		filterBI = new BeanItem<TipoClienteFiltro>(new TipoClienteFiltro());
		init(false);
		setFocusGrid();
	}

	public WLTipoCliente(TipoClienteFiltro filtro) {
		super();		
		filterBI = new BeanItem<TipoClienteFiltro>(filtro);
		init(true);
		setFocusGrid();
	}
	
	protected TipoClienteService getService() throws Exception {
		if(service == null){
			service = AppCX.services().buildTipoClienteService();
		}
	...
	protected void buildContent() throws Exception {
		...
		confWinList(this, new TipoCliente().labelPlural());
	...
	protected BeanItemContainer<TipoCliente> getItemsBIC() {
		...
		if (itemsBIC == null) {
			...
			itemsBIC = new BeanItemContainer<TipoCliente>(TipoCliente.class, new ArrayList<TipoCliente>());
		}
	...
	protected void addBeansToItemsBIC(boolean removeAllItems) {
		...
		try {
			...
			// List<TipoCliente> items = new TipoCliente().find(limit, offset, buildOrderBy(),
			// filterBI.getBean());
			...
			if (filterBI.getBean().equals(lastFilter) == false) {						
				...
				lastFilter = (TipoClienteFiltro) filterBI.getBean().clone();
				...
				List<TipoCliente> items = getService().find(filterBI.getBean());
				
				// Agrega los resultados a la grilla
				for (TipoCliente item : items) {
	...
	protected WindowForm buildWinddowForm(String mode, String id) throws Exception {
		return AppCX.widgets().buildWFTipoCliente(mode, id);
	}
...
// GENERATED BY ANTHILL 2019-06-13T18:21:20.894-03:00[America/Buenos_Aires]