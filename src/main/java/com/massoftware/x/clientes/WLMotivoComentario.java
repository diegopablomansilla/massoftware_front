...
public class WLMotivoComentario extends WindowListado {
	...
	protected MotivoComentarioService getService() throws Exception {
		if(service == null){
			service = AppCX.services().buildMotivoComentarioService();
		}
	...
	protected void addBeansToItemsBIC(boolean removeAllItems) {
		...
		try {
			...
			if (filterBI.getBean().equals(lastFilter) == false) {						
				...
				List<MotivoComentario> items = getService().find(filterBI.getBean());
	...
	protected WindowForm buildWinddowForm(String mode, String id) throws Exception {
		return AppCX.widgets().buildWFMotivoComentario(mode, id);
	}
...
// GENERATED BY ANTHILL 2019-06-11T14:41:25.201-03:00[America/Buenos_Aires]