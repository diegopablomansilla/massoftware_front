...
public class DateFieldBox extends HorizontalLayout implements Validatable {
	...
	public DateFieldEntity valueTXT;
	public Button removeFilterBTN;

	@SuppressWarnings("rawtypes")
	public DateFieldBox(WindowListado window, BeanItem dtoBI, String attName, boolean timestamp) throws Exception {
		init(window, dtoBI, attName, null, timestamp);
	}

	@SuppressWarnings("rawtypes")
	public DateFieldBox(WindowListado window, BeanItem dtoBI, String attName, String inputPrompt, boolean timestamp) throws Exception {
		init(window, dtoBI, attName, inputPrompt, timestamp);
	}

	@SuppressWarnings("rawtypes")
	private void init(WindowListado window, BeanItem dtoBI, String attName, String inputPrompt, boolean timestamp) throws Exception {

		this.setSpacing(false);

		// ----------------------------------------------

		valueTXT = new DateFieldEntity(dtoBI, attName, null);		
...
//		valueTXT.setInputPrompt(inputPrompt);
		...
		valueTXT.setDescription(inputPrompt);
		...
		valueTXT.addValueChangeListener(e -> {
			try {
				...
				if (valueTXT.getValue() == null || valueTXT.getValue().toString().trim().length() == 0) {
					window.loadDataResetPaged();
				}
			} catch (Exception ex) {
				LogAndNotification.print(ex);
