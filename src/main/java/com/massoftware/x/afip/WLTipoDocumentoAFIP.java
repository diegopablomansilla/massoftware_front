...
public class WLTipoDocumentoAFIP extends WindowListado {

	// -------------------------------------------------------------

	BeanItem<TipoDocumentoAFIPFiltro> filterBI;
	...
	public WLTipoDocumentoAFIP(TipoDocumentoAFIPFiltro filtro) {
		super();		
		filterBI = new BeanItem<TipoDocumentoAFIPFiltro>(filtro);
		init(true);
		setFocusGrid();
	}
	...
	protected void addBeansToItemsBIC(boolean removeAllItems) {
		...
		try {
			...
			// List<TipoDocumentoAFIP> items = new TipoDocumentoAFIP().find(limit, offset, buildOrderBy(),
			// filterBI.getBean());
			...
			if (filterBI.getBean().equals(lastFilter) == false) {						
				...
				lastFilter = (TipoDocumentoAFIPFiltro) filterBI.getBean().clone();
				...
				List<TipoDocumentoAFIP> items = getService().find(filterBI.getBean());
	...
	protected WindowForm buildWinddowForm(String mode, String id) throws Exception {
		return AppCX.widgets().buildWFTipoDocumentoAFIP(mode, id);
	}
...
// GENERATED BY ANTHILL 2019-06-13T18:21:20.828-03:00[America/Buenos_Aires]