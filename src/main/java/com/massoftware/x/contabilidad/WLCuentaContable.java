...
public class WLCuentaContable extends WindowListado {

	// -------------------------------------------------------------

	BeanItem<CuentaContableFiltro> filterBI;
	...
	public WLCuentaContable() {
		super();		
		filterBI = new BeanItem<CuentaContableFiltro>(new CuentaContableFiltro());
		init(false);
		setFocusGrid();
	}

	public WLCuentaContable(CuentaContableFiltro filtro) {
		super();		
		filterBI = new BeanItem<CuentaContableFiltro>(filtro);
		init(true);
		setFocusGrid();
	}
	...
	protected Component buildFiltros() throws Exception {		
		...
		codigoTXTB = new TextFieldBox(this, filterBI, "codigo", "contiene las palabras ..");
	...
	protected void buildItemsGRDLayout() throws Exception {		
		...
		itemsGRD.setColumns(new Object[] { "id", "codigo", "nombre", "ejercicioContable", "integra", "cuentaJerarquia", "imputable", "ajustaPorInflacion", "cuentaContableEstado", "cuentaConApropiacion", "centroCostoContable", "cuentaAgrupadora", "porcentaje", "puntoEquilibrio", "costoVenta", "seguridadPuerta" });
		...
		UtilUI.confColumn(itemsGRD.getColumn("integra"), true, 192);

		UtilUI.confColumn(itemsGRD.getColumn("cuentaJerarquia"), true, 192);

		UtilUI.confColumn(itemsGRD.getColumn("imputable"), true, 70);

		UtilUI.confColumn(itemsGRD.getColumn("ajustaPorInflacion"), true, 70);

		UtilUI.confColumn(itemsGRD.getColumn("cuentaContableEstado"), true, 240);

		UtilUI.confColumn(itemsGRD.getColumn("cuentaConApropiacion"), true, 70);

		UtilUI.confColumn(itemsGRD.getColumn("centroCostoContable"), true, 240);

		UtilUI.confColumn(itemsGRD.getColumn("cuentaAgrupadora"), true, 240);

		UtilUI.confColumn(itemsGRD.getColumn("porcentaje"), true, 120);

		UtilUI.confColumn(itemsGRD.getColumn("puntoEquilibrio"), true, 240);

		UtilUI.confColumn(itemsGRD.getColumn("costoVenta"), true, 240);

		UtilUI.confColumn(itemsGRD.getColumn("seguridadPuerta"), true, -1);
		...
		itemsGRD.getColumn("imputable").setRenderer(new HtmlRenderer(), new StringToBooleanConverter(FontAwesome.CHECK_SQUARE_O.getHtml(),FontAwesome.SQUARE_O.getHtml()));

		itemsGRD.getColumn("ajustaPorInflacion").setRenderer(new HtmlRenderer(), new StringToBooleanConverter(FontAwesome.CHECK_SQUARE_O.getHtml(),FontAwesome.SQUARE_O.getHtml()));

		itemsGRD.getColumn("cuentaConApropiacion").setRenderer(new HtmlRenderer(), new StringToBooleanConverter(FontAwesome.CHECK_SQUARE_O.getHtml(),FontAwesome.SQUARE_O.getHtml()));
		...
		order.add(new SortOrder("codigo", SortDirection.DESCENDING));
	...
	protected void addBeansToItemsBIC(boolean removeAllItems) {
		...
		try {
			...
			// List<CuentaContable> items = new CuentaContable().find(limit, offset, buildOrderBy(),
			// filterBI.getBean());
			...
			if (filterBI.getBean().equals(lastFilter) == false) {						
				...
				lastFilter = (CuentaContableFiltro) filterBI.getBean().clone();
				...
				List<CuentaContable> items = getService().find(filterBI.getBean());
	...
	protected WindowForm buildWinddowForm(String mode, String id) throws Exception {
		return AppCX.widgets().buildWFCuentaContable(mode, id);
	}
...
// GENERATED BY ANTHILL 2019-06-20T16:20:15.155-03:00[America/Buenos_Aires]