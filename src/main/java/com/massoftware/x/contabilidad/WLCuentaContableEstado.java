...
public class WLCuentaContableEstado extends WindowListado {

	// -------------------------------------------------------------

	BeanItem<CuentaContableEstadoFiltro> filterBI;
	protected BeanItemContainer<CuentaContableEstado> itemsBIC;
	...
	public WLCuentaContableEstado() {
		super();		
		filterBI = new BeanItem<CuentaContableEstadoFiltro>(new CuentaContableEstadoFiltro());
		init(false);
		setFocusGrid();
	}

	public WLCuentaContableEstado(CuentaContableEstadoFiltro filtro) {
		super();		
		filterBI = new BeanItem<CuentaContableEstadoFiltro>(filtro);
		init(true);
		setFocusGrid();
	}
	
	protected CuentaContableEstadoService getService() throws Exception {
		if(service == null){
			service = AppCX.services().buildCuentaContableEstadoService();
		}
	...
	protected void buildContent() throws Exception {
		...
		confWinList(this, new CuentaContableEstado().labelPlural());
	...
	protected void buildItemsGRDLayout() throws Exception {		
		...
		CuentaContableEstado dto = new CuentaContableEstado();
	...
	protected BeanItemContainer<CuentaContableEstado> getItemsBIC() {
		...
		if (itemsBIC == null) {
			...
			itemsBIC = new BeanItemContainer<CuentaContableEstado>(CuentaContableEstado.class, new ArrayList<CuentaContableEstado>());
		}
	...
	protected void addBeansToItemsBIC(boolean removeAllItems) {
		...
		try {
			...
			// List<CuentaContableEstado> items = new CuentaContableEstado().find(limit, offset, buildOrderBy(),
			// filterBI.getBean());
			...
			if (filterBI.getBean().equals(lastFilter) == false) {						
				...
				lastFilter = (CuentaContableEstadoFiltro) filterBI.getBean().clone();
				...
				List<CuentaContableEstado> items = getService().find(filterBI.getBean());
	...
	protected WindowForm buildWinddowForm(String mode, String id) throws Exception {
		return AppCX.widgets().buildWFCuentaContableEstado(mode, id);
	}
...
// GENERATED BY ANTHILL 2019-06-19T11:51:57.517-03:00[America/Buenos_Aires]