...
public class WLAsientoContableModulo extends WindowListado {

	// -------------------------------------------------------------

	BeanItem<AsientoContableModuloFiltro> filterBI;
	protected BeanItemContainer<AsientoContableModulo> itemsBIC;
	...
	public WLAsientoContableModulo() {
		super();		
		filterBI = new BeanItem<AsientoContableModuloFiltro>(new AsientoContableModuloFiltro());
		init(false);
		setFocusGrid();
	}

	public WLAsientoContableModulo(AsientoContableModuloFiltro filtro) {
		super();		
		filterBI = new BeanItem<AsientoContableModuloFiltro>(filtro);
		init(true);
		setFocusGrid();
	}
	
	protected AsientoContableModuloService getService() throws Exception {
		if(service == null){
			service = AppCX.services().buildAsientoContableModuloService();
		}
	...
	protected void buildContent() throws Exception {
		...
		confWinList(this, new AsientoContableModulo().labelPlural());
	...
	protected void buildItemsGRDLayout() throws Exception {		
		...
		AsientoContableModulo dto = new AsientoContableModulo();
	...
	protected BeanItemContainer<AsientoContableModulo> getItemsBIC() {
		...
		if (itemsBIC == null) {
			...
			itemsBIC = new BeanItemContainer<AsientoContableModulo>(AsientoContableModulo.class, new ArrayList<AsientoContableModulo>());
		}
	...
	protected void addBeansToItemsBIC(boolean removeAllItems) {
		...
		try {
			...
			// List<AsientoContableModulo> items = new AsientoContableModulo().find(limit, offset, buildOrderBy(),
			// filterBI.getBean());
			...
			if (filterBI.getBean().equals(lastFilter) == false) {						
				...
				lastFilter = (AsientoContableModuloFiltro) filterBI.getBean().clone();
				...
				List<AsientoContableModulo> items = getService().find(filterBI.getBean());
				
				// Agrega los resultados a la grilla
				for (AsientoContableModulo item : items) {
	...
	protected WindowForm buildWinddowForm(String mode, String id) throws Exception {
		return AppCX.widgets().buildWFAsientoContableModulo(mode, id);
	}
...
// GENERATED BY ANTHILL 2019-06-19T11:23:56.545-03:00[America/Buenos_Aires]