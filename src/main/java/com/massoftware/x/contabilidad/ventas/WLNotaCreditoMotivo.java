...
public class WLNotaCreditoMotivo extends WindowListado {

	// -------------------------------------------------------------

	BeanItem<NotaCreditoMotivoFiltro> filterBI;
	protected BeanItemContainer<NotaCreditoMotivo> itemsBIC;
	...
	public WLNotaCreditoMotivo() {
		super();		
		filterBI = new BeanItem<NotaCreditoMotivoFiltro>(new NotaCreditoMotivoFiltro());
		init(false);
		setFocusGrid();
	}

	public WLNotaCreditoMotivo(NotaCreditoMotivoFiltro filtro) {
		super();		
		filterBI = new BeanItem<NotaCreditoMotivoFiltro>(filtro);
		init(true);
		setFocusGrid();
	}
	
	protected NotaCreditoMotivoService getService() throws Exception {
		if(service == null){
			service = AppCX.services().buildNotaCreditoMotivoService();
		}
	...
	protected void buildContent() throws Exception {
		...
		confWinList(this, new NotaCreditoMotivo().labelPlural());
	...
	protected BeanItemContainer<NotaCreditoMotivo> getItemsBIC() {
		...
		if (itemsBIC == null) {
			...
			itemsBIC = new BeanItemContainer<NotaCreditoMotivo>(NotaCreditoMotivo.class, new ArrayList<NotaCreditoMotivo>());
		}
	...
	protected void addBeansToItemsBIC(boolean removeAllItems) {
		...
		try {
			...
			// List<NotaCreditoMotivo> items = new NotaCreditoMotivo().find(limit, offset, buildOrderBy(),
			// filterBI.getBean());
			...
			if (filterBI.getBean().equals(lastFilter) == false) {						
				...
				lastFilter = (NotaCreditoMotivoFiltro) filterBI.getBean().clone();
				...
				List<NotaCreditoMotivo> items = getService().find(filterBI.getBean());
				
				// Agrega los resultados a la grilla
				for (NotaCreditoMotivo item : items) {
	...
	protected WindowForm buildWinddowForm(String mode, String id) throws Exception {
		return AppCX.widgets().buildWFNotaCreditoMotivo(mode, id);
	}
...
// GENERATED BY ANTHILL 2019-06-13T18:21:20.861-03:00[America/Buenos_Aires]