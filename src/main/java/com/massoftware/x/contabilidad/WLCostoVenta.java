...
public class WLCostoVenta extends WindowListado {

	// -------------------------------------------------------------

	BeanItem<CostoVentaFiltro> filterBI;
	protected BeanItemContainer<CostoVenta> itemsBIC;
	...
	public WLCostoVenta() {
		super();		
		filterBI = new BeanItem<CostoVentaFiltro>(new CostoVentaFiltro());
		init(false);
		setFocusGrid();
	}

	public WLCostoVenta(CostoVentaFiltro filtro) {
		super();		
		filterBI = new BeanItem<CostoVentaFiltro>(filtro);
		init(true);
		setFocusGrid();
	}
	
	protected CostoVentaService getService() throws Exception {
		if(service == null){
			service = AppCX.services().buildCostoVentaService();
		}
	...
	protected void buildContent() throws Exception {
		...
		confWinList(this, new CostoVenta().labelPlural());
	...
	protected BeanItemContainer<CostoVenta> getItemsBIC() {
		...
		if (itemsBIC == null) {
			...
			itemsBIC = new BeanItemContainer<CostoVenta>(CostoVenta.class, new ArrayList<CostoVenta>());
		}
	...
	protected void addBeansToItemsBIC(boolean removeAllItems) {
		...
		try {
			...
			// List<CostoVenta> items = new CostoVenta().find(limit, offset, buildOrderBy(),
			// filterBI.getBean());
			...
			if (filterBI.getBean().equals(lastFilter) == false) {						
				...
				lastFilter = (CostoVentaFiltro) filterBI.getBean().clone();
				...
				List<CostoVenta> items = getService().find(filterBI.getBean());
				
				// Agrega los resultados a la grilla
				for (CostoVenta item : items) {
	...
	protected WindowForm buildWinddowForm(String mode, String id) throws Exception {
		return AppCX.widgets().buildWFCostoVenta(mode, id);
	}
...
// GENERATED BY ANTHILL 2019-06-19T12:29:54.456-03:00[America/Buenos_Aires]