...
public class WLEjercicioContable extends WindowListado {

	// -------------------------------------------------------------

	BeanItem<EjercicioContableFiltro> filterBI;
	...
	public WLEjercicioContable() {
		super();		
		filterBI = new BeanItem<EjercicioContableFiltro>(new EjercicioContableFiltro());
		init(false);
		setFocusGrid();
	}

	public WLEjercicioContable(EjercicioContableFiltro filtro) {
		super();		
		filterBI = new BeanItem<EjercicioContableFiltro>(filtro);
		init(true);
		setFocusGrid();
	}
	...
	protected void buildItemsGRDLayout() throws Exception {		
		...
		itemsGRD.setColumns(new Object[] { "id", "numero", "apertura", "cierre", "cerrado", "cerradoModulos", "comentario" });
		...
		UtilUI.confColumn(itemsGRD.getColumn("apertura"), true, 240);

		UtilUI.confColumn(itemsGRD.getColumn("cierre"), true, 240);

		UtilUI.confColumn(itemsGRD.getColumn("cerrado"), true, 70);

		UtilUI.confColumn(itemsGRD.getColumn("cerradoModulos"), true, 70);
		...
		itemsGRD.getColumn("cerrado").setRenderer(new HtmlRenderer(), new StringToBooleanConverter(FontAwesome.CHECK_SQUARE_O.getHtml(),FontAwesome.SQUARE_O.getHtml()));

		itemsGRD.getColumn("cerradoModulos").setRenderer(new HtmlRenderer(), new StringToBooleanConverter(FontAwesome.CHECK_SQUARE_O.getHtml(),FontAwesome.SQUARE_O.getHtml()));

		// SI UNA COLUMNA ES DE TIPO DATE HACER LO QUE SIGUE
		// itemsGRD.getColumn("attName").setRenderer(new DateRenderer(new SimpleDateFormat("dd/MM/yyyy")));
		

		itemsGRD.getColumn("apertura").setRenderer(new DateRenderer(new SimpleDateFormat("dd/MM/yyyy")));
	...
	protected BeanItemContainer<EjercicioContable> getItemsBIC() {
		...
		if (itemsBIC == null) {
			...
			itemsBIC = new BeanItemContainer<EjercicioContable>(EjercicioContable.class, new ArrayList<EjercicioContable>());
		}
	...
	protected void addBeansToItemsBIC(boolean removeAllItems) {
		...
		try {
			...
			if (filterBI.getBean().equals(lastFilter) == false) {						
				...
				lastFilter = (EjercicioContableFiltro) filterBI.getBean().clone();
				...
				List<EjercicioContable> items = getService().find(filterBI.getBean());
	...
	protected WindowForm buildWinddowForm(String mode, String id) throws Exception {
		return AppCX.widgets().buildWFEjercicioContable(mode, id);
	}
...
// GENERATED BY ANTHILL 2019-06-20T16:41:26.664-03:00[America/Buenos_Aires]