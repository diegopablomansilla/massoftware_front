...
public class WLTipoPuntoEquilibrio extends WindowListado {

	// -------------------------------------------------------------

	BeanItem<TipoPuntoEquilibrioFiltro> filterBI;
	protected BeanItemContainer<TipoPuntoEquilibrio> itemsBIC;
	...
	public WLTipoPuntoEquilibrio() {
		super();		
		filterBI = new BeanItem<TipoPuntoEquilibrioFiltro>(new TipoPuntoEquilibrioFiltro());
		init(false);
		setFocusGrid();
	}

	public WLTipoPuntoEquilibrio(TipoPuntoEquilibrioFiltro filtro) {
		super();		
		filterBI = new BeanItem<TipoPuntoEquilibrioFiltro>(filtro);
		init(true);
		setFocusGrid();
	}
	
	protected TipoPuntoEquilibrioService getService() throws Exception {
		if(service == null){
			service = AppCX.services().buildTipoPuntoEquilibrioService();
		}
	...
	protected void buildContent() throws Exception {
		...
		confWinList(this, new TipoPuntoEquilibrio().labelPlural());
	...
	protected void buildItemsGRDLayout() throws Exception {		
		...
		TipoPuntoEquilibrio dto = new TipoPuntoEquilibrio();
	...
	protected BeanItemContainer<TipoPuntoEquilibrio> getItemsBIC() {
		...
		if (itemsBIC == null) {
			...
			itemsBIC = new BeanItemContainer<TipoPuntoEquilibrio>(TipoPuntoEquilibrio.class, new ArrayList<TipoPuntoEquilibrio>());
		}
	...
	protected void addBeansToItemsBIC(boolean removeAllItems) {
		...
		try {
			...
			// List<TipoPuntoEquilibrio> items = new TipoPuntoEquilibrio().find(limit, offset, buildOrderBy(),
			// filterBI.getBean());
			...
			if (filterBI.getBean().equals(lastFilter) == false) {						
				...
				lastFilter = (TipoPuntoEquilibrioFiltro) filterBI.getBean().clone();
				...
				List<TipoPuntoEquilibrio> items = getService().find(filterBI.getBean());
				
				// Agrega los resultados a la grilla
				for (TipoPuntoEquilibrio item : items) {
	...
	protected WindowForm buildWinddowForm(String mode, String id) throws Exception {
		return AppCX.widgets().buildWFTipoPuntoEquilibrio(mode, id);
	}
...
// GENERATED BY ANTHILL 2019-06-19T12:29:54.430-03:00[America/Buenos_Aires]