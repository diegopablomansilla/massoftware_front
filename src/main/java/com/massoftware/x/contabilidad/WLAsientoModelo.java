...
public class WLAsientoModelo extends WindowListado {

	// -------------------------------------------------------------

	BeanItem<AsientoModeloFiltro> filterBI;
	...
	public WLAsientoModelo() {
		super();		
		filterBI = new BeanItem<AsientoModeloFiltro>(new AsientoModeloFiltro());
		init(false);
		setFocusGrid();
	}

	public WLAsientoModelo(AsientoModeloFiltro filtro) {
		super();		
		filterBI = new BeanItem<AsientoModeloFiltro>(filtro);
		init(true);
		setFocusGrid();
	}
	...
	protected void buildItemsGRDLayout() throws Exception {		
		...
		itemsGRD.setColumns(new Object[] { "id", "numero", "nombre", "ejercicioContable" });
	...
	protected void addBeansToItemsBIC(boolean removeAllItems) {
		...
		try {
			...
			// List<AsientoModelo> items = new AsientoModelo().find(limit, offset, buildOrderBy(),
			// filterBI.getBean());
			...
			if (filterBI.getBean().equals(lastFilter) == false) {						
				...
				lastFilter = (AsientoModeloFiltro) filterBI.getBean().clone();
				...
				List<AsientoModelo> items = getService().find(filterBI.getBean());
	...
	protected WindowForm buildWinddowForm(String mode, String id) throws Exception {
		return AppCX.widgets().buildWFAsientoModelo(mode, id);
	}
...
// GENERATED BY ANTHILL 2019-06-19T11:23:56.468-03:00[America/Buenos_Aires]