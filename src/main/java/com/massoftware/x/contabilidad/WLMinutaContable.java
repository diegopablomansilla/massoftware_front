...
public class WLMinutaContable extends WindowListado {

	// -------------------------------------------------------------

	BeanItem<MinutaContableFiltro> filterBI;
	protected BeanItemContainer<MinutaContable> itemsBIC;
	...
	public WLMinutaContable() {
		super();		
		filterBI = new BeanItem<MinutaContableFiltro>(new MinutaContableFiltro());
		init(false);
		setFocusGrid();
	}

	public WLMinutaContable(MinutaContableFiltro filtro) {
		super();		
		filterBI = new BeanItem<MinutaContableFiltro>(filtro);
		init(true);
		setFocusGrid();
	}
	
	protected MinutaContableService getService() throws Exception {
		if(service == null){
			service = AppCX.services().buildMinutaContableService();
		}
	...
	protected void buildContent() throws Exception {
		...
		confWinList(this, new MinutaContable().labelPlural());
	...
	protected void buildItemsGRDLayout() throws Exception {		
		...
		MinutaContable dto = new MinutaContable();
	...
	protected BeanItemContainer<MinutaContable> getItemsBIC() {
		...
		if (itemsBIC == null) {
			...
			itemsBIC = new BeanItemContainer<MinutaContable>(MinutaContable.class, new ArrayList<MinutaContable>());
		}
	...
	protected void addBeansToItemsBIC(boolean removeAllItems) {
		...
		try {
			...
			// List<MinutaContable> items = new MinutaContable().find(limit, offset, buildOrderBy(),
			// filterBI.getBean());
			...
			if (filterBI.getBean().equals(lastFilter) == false) {						
				...
				lastFilter = (MinutaContableFiltro) filterBI.getBean().clone();
				...
				List<MinutaContable> items = getService().find(filterBI.getBean());
				
				// Agrega los resultados a la grilla
				for (MinutaContable item : items) {
	...
	protected WindowForm buildWinddowForm(String mode, String id) throws Exception {
		return AppCX.widgets().buildWFMinutaContable(mode, id);
	}
...
// GENERATED BY ANTHILL 2019-06-20T16:20:15.288-03:00[America/Buenos_Aires]