...
public class WLCuentaFondoGrupo extends WindowListado {

	// -------------------------------------------------------------

	BeanItem<CuentaFondoGrupoFiltro> filterBI;
	protected BeanItemContainer<CuentaFondoGrupo> itemsBIC;
	...
	protected ComboBoxBox cuentaFondoRubroCBXB;
	protected SelectorBox cuentaFondoRubroSBX;


	// -------------------------------------------------------------

	public WLCuentaFondoGrupo() {
		super();		
		filterBI = new BeanItem<CuentaFondoGrupoFiltro>(new CuentaFondoGrupoFiltro());
		init(false);
		setFocusGrid();
	}

	public WLCuentaFondoGrupo(CuentaFondoGrupoFiltro filtro) {
		super();		
		filterBI = new BeanItem<CuentaFondoGrupoFiltro>(filtro);
		init(true);
		setFocusGrid();
	}
	...
	protected void buildContent() throws Exception {
		...
		confWinList(this, new CuentaFondoGrupo().labelPlural());
	...
	protected Component buildFiltros() throws Exception {		
		...
		if (cuentaFondoRubroItems < MAX_ROWS_FOR_CBX) {
			...
			cuentaFondoRubroCBXB = new ComboBoxBox(this, filterBI, "cuentaFondoRubro", cuentaFondoRubroLista, filterBI.getBean().getCuentaFondoRubro());

		} else {

			cuentaFondoRubroSBX = new SelectorBox(filterBI, "cuentaFondoRubro") {
				...
				protected WindowListado getPopup(boolean filter) throws Exception {
					...
					//WLCuentaFondoRubro windowPoPup = new WLCuentaFondoRubro(filtro) {

						//protected void setSelectedItem() throws Exception {

							//cuentaFondoRubroSBX.setSelectedItem(itemsGRD.getSelectedRow());

						//}

					//};

					WLCuentaFondoRubro windowPoPup = AppCX.widgets().buildWLCuentaFondoRubro(filtro);

					windowPoPup.selectorSBX = cuentaFondoRubroSBX;
	...
	protected Component buildFiltrosLayout() throws Exception {	
		...
		if (cuentaFondoRubroCBXB != null) {
			filaFiltroHL.addComponent(cuentaFondoRubroCBXB);
		}
		if (cuentaFondoRubroSBX != null) {
			filaFiltroHL.addComponent(cuentaFondoRubroSBX);
		}
	...
	protected void buildItemsGRDLayout() throws Exception {		
		...
		itemsGRD.setColumns(new Object[] { "id", "numero", "nombre", "cuentaFondoRubro" });
		...
		UtilUI.confColumn(itemsGRD.getColumn("cuentaFondoRubro"), true, -1);
		
		// ------------------------------------------------------------------

		CuentaFondoGrupo dto = new CuentaFondoGrupo();
	...
	protected BeanItemContainer<CuentaFondoGrupo> getItemsBIC() {
		...
		if (itemsBIC == null) {
			...
			itemsBIC = new BeanItemContainer<CuentaFondoGrupo>(CuentaFondoGrupo.class, new ArrayList<CuentaFondoGrupo>());
		}
	...
	protected void addBeansToItemsBIC(boolean removeAllItems) {
		...
		try {
			...
			// List<CuentaFondoGrupo> items = new CuentaFondoGrupo().find(limit, offset, buildOrderBy(),
			// filterBI.getBean());
			...
			if (filterBI.getBean().equals(lastFilter) == false) {						
				...
				lastFilter = (CuentaFondoGrupoFiltro) filterBI.getBean().clone();
				...
				List<CuentaFondoGrupo> items = getService().find(filterBI.getBean());
				
				// Agrega los resultados a la grilla
				for (CuentaFondoGrupo item : items) {
	...
	protected WindowForm buildWinddowForm(String mode, String id) throws Exception {
		return AppCX.widgets().buildWFCuentaFondoGrupo(mode, id);
	}
...
// GENERATED BY ANTHILL 2019-07-08T12:47:40.822-03:00[America/Buenos_Aires]