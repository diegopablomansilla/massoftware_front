...
public class WLComportamientoComprobante extends WindowListado {

	// -------------------------------------------------------------

	BeanItem<ComportamientoComprobanteFiltro> filterBI;
	protected BeanItemContainer<ComportamientoComprobante> itemsBIC;
	...
	public WLComportamientoComprobante() {
		super();		
		filterBI = new BeanItem<ComportamientoComprobanteFiltro>(new ComportamientoComprobanteFiltro());
		init(false);
		setFocusGrid();
	}

	public WLComportamientoComprobante(ComportamientoComprobanteFiltro filtro) {
		super();		
		filterBI = new BeanItem<ComportamientoComprobanteFiltro>(filtro);
		init(true);
		setFocusGrid();
	}
	
	protected ComportamientoComprobanteService getService() throws Exception {
		if(service == null){
			service = AppCX.services().buildComportamientoComprobanteService();
		}
	...
	protected void buildContent() throws Exception {
		...
		confWinList(this, new ComportamientoComprobante().labelPlural());
	...
	protected void buildItemsGRDLayout() throws Exception {		
		...
		ComportamientoComprobante dto = new ComportamientoComprobante();
	...
	protected BeanItemContainer<ComportamientoComprobante> getItemsBIC() {
		...
		if (itemsBIC == null) {
			...
			itemsBIC = new BeanItemContainer<ComportamientoComprobante>(ComportamientoComprobante.class, new ArrayList<ComportamientoComprobante>());
		}
	...
	protected void addBeansToItemsBIC(boolean removeAllItems) {
		...
		try {
			...
			// List<ComportamientoComprobante> items = new ComportamientoComprobante().find(limit, offset, buildOrderBy(),
			// filterBI.getBean());
			...
			if (filterBI.getBean().equals(lastFilter) == false) {						
				...
				lastFilter = (ComportamientoComprobanteFiltro) filterBI.getBean().clone();
				...
				List<ComportamientoComprobante> items = getService().find(filterBI.getBean());
				
				// Agrega los resultados a la grilla
				for (ComportamientoComprobante item : items) {
	...
	protected WindowForm buildWinddowForm(String mode, String id) throws Exception {
		return AppCX.widgets().buildWFComportamientoComprobante(mode, id);
	}
...
// GENERATED BY ANTHILL 2019-07-08T12:47:41.605-03:00[America/Buenos_Aires]