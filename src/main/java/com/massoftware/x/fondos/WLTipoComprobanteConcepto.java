...
public class WLTipoComprobanteConcepto extends WindowListado {

	// -------------------------------------------------------------

	BeanItem<TipoComprobanteConceptoFiltro> filterBI;
	protected BeanItemContainer<TipoComprobanteConcepto> itemsBIC;
	...
	public WLTipoComprobanteConcepto() {
		super();		
		filterBI = new BeanItem<TipoComprobanteConceptoFiltro>(new TipoComprobanteConceptoFiltro());
		init(false);
		setFocusGrid();
	}

	public WLTipoComprobanteConcepto(TipoComprobanteConceptoFiltro filtro) {
		super();		
		filterBI = new BeanItem<TipoComprobanteConceptoFiltro>(filtro);
		init(true);
		setFocusGrid();
	}
	
	protected TipoComprobanteConceptoService getService() throws Exception {
		if(service == null){
			service = AppCX.services().buildTipoComprobanteConceptoService();
		}
	...
	protected void buildContent() throws Exception {
		...
		confWinList(this, new TipoComprobanteConcepto().labelPlural());
	...
	protected void buildItemsGRDLayout() throws Exception {		
		...
		TipoComprobanteConcepto dto = new TipoComprobanteConcepto();
	...
	protected BeanItemContainer<TipoComprobanteConcepto> getItemsBIC() {
		...
		if (itemsBIC == null) {
			...
			itemsBIC = new BeanItemContainer<TipoComprobanteConcepto>(TipoComprobanteConcepto.class, new ArrayList<TipoComprobanteConcepto>());
		}
	...
	protected void addBeansToItemsBIC(boolean removeAllItems) {
		...
		try {
			...
			// List<TipoComprobanteConcepto> items = new TipoComprobanteConcepto().find(limit, offset, buildOrderBy(),
			// filterBI.getBean());
			...
			if (filterBI.getBean().equals(lastFilter) == false) {						
				...
				lastFilter = (TipoComprobanteConceptoFiltro) filterBI.getBean().clone();
				...
				List<TipoComprobanteConcepto> items = getService().find(filterBI.getBean());
				
				// Agrega los resultados a la grilla
				for (TipoComprobanteConcepto item : items) {
	...
	protected WindowForm buildWinddowForm(String mode, String id) throws Exception {
		return AppCX.widgets().buildWFTipoComprobanteConcepto(mode, id);
	}
...
// GENERATED BY ANTHILL 2019-06-28T11:36:37.383-03:00[America/Buenos_Aires]