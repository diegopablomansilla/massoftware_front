...
public class WLJuridiccionConvnioMultilateral extends WindowListado {
	...
	public WLJuridiccionConvnioMultilateral() {
		super();		
	...
	public WLJuridiccionConvnioMultilateral(JuridiccionConvnioMultilateralFiltro filtro) {
		super();		
	...
	protected void buildItemsGRDLayout() throws Exception {		
		...
		itemsGRD.setColumns(new Object[] { "id", "numero", "nombre", "cuentaFondo" });
	...
	protected BeanItemContainer<JuridiccionConvnioMultilateral> getItemsBIC() {
		...
		if (itemsBIC == null) {
			...
			itemsBIC = new BeanItemContainer<JuridiccionConvnioMultilateral>(JuridiccionConvnioMultilateral.class, new ArrayList<JuridiccionConvnioMultilateral>());
		}
	...
	protected void addBeansToItemsBIC(boolean removeAllItems) {
		...
		try {
			...
			// List<JuridiccionConvnioMultilateral> items = new JuridiccionConvnioMultilateral().find(limit, offset, buildOrderBy(),
			// filterBI.getBean());
			...
			if (filterBI.getBean().equals(lastFilter) == false) {						
				...
				lastFilter = (JuridiccionConvnioMultilateralFiltro) filterBI.getBean().clone();
				...
				List<JuridiccionConvnioMultilateral> items = getService().find(filterBI.getBean());
	...
	protected WindowForm buildWinddowForm(String mode, String id) throws Exception {
		return AppCX.widgets().buildWFJuridiccionConvnioMultilateral(mode, id);
	}
...
// GENERATED BY ANTHILL 2019-06-28T11:36:37.267-03:00[America/Buenos_Aires]