...
public class WLComprobanteFondoModelo extends WindowListado {

	// -------------------------------------------------------------

	BeanItem<ComprobanteFondoModeloFiltro> filterBI;
	protected BeanItemContainer<ComprobanteFondoModelo> itemsBIC;
	...
	public WLComprobanteFondoModelo() {
		super();		
		filterBI = new BeanItem<ComprobanteFondoModeloFiltro>(new ComprobanteFondoModeloFiltro());
		init(false);
		setFocusGrid();
	}

	public WLComprobanteFondoModelo(ComprobanteFondoModeloFiltro filtro) {
		super();		
		filterBI = new BeanItem<ComprobanteFondoModeloFiltro>(filtro);
		init(true);
		setFocusGrid();
	}
	
	protected ComprobanteFondoModeloService getService() throws Exception {
		if(service == null){
			service = AppCX.services().buildComprobanteFondoModeloService();
		}
	...
	protected void buildContent() throws Exception {
		...
		confWinList(this, new ComprobanteFondoModelo().labelPlural());
	...
	protected void buildItemsGRDLayout() throws Exception {		
		...
		ComprobanteFondoModelo dto = new ComprobanteFondoModelo();
	...
	protected BeanItemContainer<ComprobanteFondoModelo> getItemsBIC() {
		...
		if (itemsBIC == null) {
			...
			itemsBIC = new BeanItemContainer<ComprobanteFondoModelo>(ComprobanteFondoModelo.class, new ArrayList<ComprobanteFondoModelo>());
		}
	...
	protected void addBeansToItemsBIC(boolean removeAllItems) {
		...
		try {
			...
			// List<ComprobanteFondoModelo> items = new ComprobanteFondoModelo().find(limit, offset, buildOrderBy(),
			// filterBI.getBean());
			...
			if (filterBI.getBean().equals(lastFilter) == false) {						
				...
				lastFilter = (ComprobanteFondoModeloFiltro) filterBI.getBean().clone();
				...
				List<ComprobanteFondoModelo> items = getService().find(filterBI.getBean());
				
				// Agrega los resultados a la grilla
				for (ComprobanteFondoModelo item : items) {
	...
	protected WindowForm buildWinddowForm(String mode, String id) throws Exception {
		return AppCX.widgets().buildWFComprobanteFondoModelo(mode, id);
	}
...
// GENERATED BY ANTHILL 2019-06-20T16:41:27.443-03:00[America/Buenos_Aires]