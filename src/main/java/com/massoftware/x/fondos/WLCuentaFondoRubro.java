...
public class WLCuentaFondoRubro extends WindowListado {

	// -------------------------------------------------------------

	BeanItem<CuentaFondoRubroFiltro> filterBI;
	protected BeanItemContainer<CuentaFondoRubro> itemsBIC;
	...
	public WLCuentaFondoRubro() {
		super();		
		filterBI = new BeanItem<CuentaFondoRubroFiltro>(new CuentaFondoRubroFiltro());
		init(false);
		setFocusGrid();
	}

	public WLCuentaFondoRubro(CuentaFondoRubroFiltro filtro) {
		super();		
		filterBI = new BeanItem<CuentaFondoRubroFiltro>(filtro);
		init(true);
		setFocusGrid();
	}
	
	protected CuentaFondoRubroService getService() throws Exception {
		if(service == null){
			service = AppCX.services().buildCuentaFondoRubroService();
		}
	...
	protected void buildContent() throws Exception {
		...
		confWinList(this, new CuentaFondoRubro().labelPlural());
	...
	protected void buildItemsGRDLayout() throws Exception {		
		...
		CuentaFondoRubro dto = new CuentaFondoRubro();
	...
	protected BeanItemContainer<CuentaFondoRubro> getItemsBIC() {
		...
		if (itemsBIC == null) {
			...
			itemsBIC = new BeanItemContainer<CuentaFondoRubro>(CuentaFondoRubro.class, new ArrayList<CuentaFondoRubro>());
		}
	...
	protected void addBeansToItemsBIC(boolean removeAllItems) {
		...
		try {
			...
			// List<CuentaFondoRubro> items = new CuentaFondoRubro().find(limit, offset, buildOrderBy(),
			// filterBI.getBean());
			...
			if (filterBI.getBean().equals(lastFilter) == false) {						
				...
				lastFilter = (CuentaFondoRubroFiltro) filterBI.getBean().clone();
				...
				List<CuentaFondoRubro> items = getService().find(filterBI.getBean());
				
				// Agrega los resultados a la grilla
				for (CuentaFondoRubro item : items) {
	...
	protected WindowForm buildWinddowForm(String mode, String id) throws Exception {
		return AppCX.widgets().buildWFCuentaFondoRubro(mode, id);
	}
...
// GENERATED BY ANTHILL 2019-06-28T11:36:36.602-03:00[America/Buenos_Aires]