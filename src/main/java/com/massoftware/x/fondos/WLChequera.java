...
public class WLChequera extends WindowListado {

	// -------------------------------------------------------------

	BeanItem<ChequeraFiltro> filterBI;
	protected BeanItemContainer<Chequera> itemsBIC;
	...
	public WLChequera() {
		super();		
		filterBI = new BeanItem<ChequeraFiltro>(new ChequeraFiltro());
		init(false);
		setFocusGrid();
	}

	public WLChequera(ChequeraFiltro filtro) {
		super();		
		filterBI = new BeanItem<ChequeraFiltro>(filtro);
		init(true);
		setFocusGrid();
	}
	...
	protected void buildContent() throws Exception {
		...
		confWinList(this, new Chequera().labelPlural());
	...
	protected void buildItemsGRDLayout() throws Exception {		
		...
		itemsGRD.setColumns(new Object[] { "id", "numero", "nombre", "cuentaFondo", "primerNumero", "ultimoNumero", "proximoNumero", "bloqueado", "impresionDiferida", "formato" });
		...
		UtilUI.confColumn(itemsGRD.getColumn("cuentaFondo"), true, 240);
		...
		UtilUI.confColumn(itemsGRD.getColumn("impresionDiferida"), true, 70);

		UtilUI.confColumn(itemsGRD.getColumn("formato"), true, -1);
		
		// ------------------------------------------------------------------

		Chequera dto = new Chequera();
		...
		itemsGRD.getColumn("impresionDiferida").setRenderer(new HtmlRenderer(), new StringToBooleanConverter(FontAwesome.CHECK_SQUARE_O.getHtml(),FontAwesome.SQUARE_O.getHtml()));
	...
	protected BeanItemContainer<Chequera> getItemsBIC() {
		...
		if (itemsBIC == null) {
			...
			itemsBIC = new BeanItemContainer<Chequera>(Chequera.class, new ArrayList<Chequera>());
		}
	...
	protected void addBeansToItemsBIC(boolean removeAllItems) {
		...
		try {
			...
			// List<Chequera> items = new Chequera().find(limit, offset, buildOrderBy(),
			// filterBI.getBean());
			...
			if (filterBI.getBean().equals(lastFilter) == false) {						
				...
				lastFilter = (ChequeraFiltro) filterBI.getBean().clone();
				...
				List<Chequera> items = getService().find(filterBI.getBean());
				
				// Agrega los resultados a la grilla
				for (Chequera item : items) {
	...
	protected WindowForm buildWinddowForm(String mode, String id) throws Exception {
		return AppCX.widgets().buildWFChequera(mode, id);
	}
...
// GENERATED BY ANTHILL 2019-06-28T11:36:37.359-03:00[America/Buenos_Aires]