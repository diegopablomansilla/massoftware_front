...
public class WLTicketControlDenunciados extends WindowListado {

	// -------------------------------------------------------------

	BeanItem<TicketControlDenunciadosFiltro> filterBI;
	protected BeanItemContainer<TicketControlDenunciados> itemsBIC;
	...
	public WLTicketControlDenunciados() {
		super();		
		filterBI = new BeanItem<TicketControlDenunciadosFiltro>(new TicketControlDenunciadosFiltro());
		init(false);
		setFocusGrid();
	}

	public WLTicketControlDenunciados(TicketControlDenunciadosFiltro filtro) {
		super();		
		filterBI = new BeanItem<TicketControlDenunciadosFiltro>(filtro);
		init(true);
		setFocusGrid();
	}
	
	protected TicketControlDenunciadosService getService() throws Exception {
		if(service == null){
			service = AppCX.services().buildTicketControlDenunciadosService();
		}
	...
	protected void buildContent() throws Exception {
		...
		confWinList(this, new TicketControlDenunciados().labelPlural());
	...
	protected void buildItemsGRDLayout() throws Exception {		
		...
		TicketControlDenunciados dto = new TicketControlDenunciados();
	...
	protected BeanItemContainer<TicketControlDenunciados> getItemsBIC() {
		...
		if (itemsBIC == null) {
			...
			itemsBIC = new BeanItemContainer<TicketControlDenunciados>(TicketControlDenunciados.class, new ArrayList<TicketControlDenunciados>());
		}
	...
	protected void addBeansToItemsBIC(boolean removeAllItems) {
		...
		try {
			...
			// List<TicketControlDenunciados> items = new TicketControlDenunciados().find(limit, offset, buildOrderBy(),
			// filterBI.getBean());
			...
			if (filterBI.getBean().equals(lastFilter) == false) {						
				...
				lastFilter = (TicketControlDenunciadosFiltro) filterBI.getBean().clone();
				...
				List<TicketControlDenunciados> items = getService().find(filterBI.getBean());
				
				// Agrega los resultados a la grilla
				for (TicketControlDenunciados item : items) {
	...
	protected WindowForm buildWinddowForm(String mode, String id) throws Exception {
		return AppCX.widgets().buildWFTicketControlDenunciados(mode, id);
	}
...
// GENERATED BY ANTHILL 2019-07-08T12:47:41.221-03:00[America/Buenos_Aires]