...
public class WLTipoComprobanteCopia extends WindowListado {

	// -------------------------------------------------------------

	BeanItem<TipoComprobanteCopiaFiltro> filterBI;
	protected BeanItemContainer<TipoComprobanteCopia> itemsBIC;
	...
	public WLTipoComprobanteCopia() {
		super();		
		filterBI = new BeanItem<TipoComprobanteCopiaFiltro>(new TipoComprobanteCopiaFiltro());
		init(false);
		setFocusGrid();
	}

	public WLTipoComprobanteCopia(TipoComprobanteCopiaFiltro filtro) {
		super();		
		filterBI = new BeanItem<TipoComprobanteCopiaFiltro>(filtro);
		init(true);
		setFocusGrid();
	}
	
	protected TipoComprobanteCopiaService getService() throws Exception {
		if(service == null){
			service = AppCX.services().buildTipoComprobanteCopiaService();
		}
	...
	protected void buildContent() throws Exception {
		...
		confWinList(this, new TipoComprobanteCopia().labelPlural());
	...
	protected void buildItemsGRDLayout() throws Exception {		
		...
		TipoComprobanteCopia dto = new TipoComprobanteCopia();
	...
	protected BeanItemContainer<TipoComprobanteCopia> getItemsBIC() {
		...
		if (itemsBIC == null) {
			...
			itemsBIC = new BeanItemContainer<TipoComprobanteCopia>(TipoComprobanteCopia.class, new ArrayList<TipoComprobanteCopia>());
		}
	...
	protected void addBeansToItemsBIC(boolean removeAllItems) {
		...
		try {
			...
			// List<TipoComprobanteCopia> items = new TipoComprobanteCopia().find(limit, offset, buildOrderBy(),
			// filterBI.getBean());
			...
			if (filterBI.getBean().equals(lastFilter) == false) {						
				...
				lastFilter = (TipoComprobanteCopiaFiltro) filterBI.getBean().clone();
				...
				List<TipoComprobanteCopia> items = getService().find(filterBI.getBean());
				
				// Agrega los resultados a la grilla
				for (TipoComprobanteCopia item : items) {
	...
	protected WindowForm buildWinddowForm(String mode, String id) throws Exception {
		return AppCX.widgets().buildWFTipoComprobanteCopia(mode, id);
	}
...
// GENERATED BY ANTHILL 2019-07-08T12:47:41.620-03:00[America/Buenos_Aires]