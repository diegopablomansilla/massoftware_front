...
public class WLCuentaFondoTipo extends WindowListado {

	// -------------------------------------------------------------

	BeanItem<CuentaFondoTipoFiltro> filterBI;
	protected BeanItemContainer<CuentaFondoTipo> itemsBIC;
	...
	public WLCuentaFondoTipo() {
		super();		
		filterBI = new BeanItem<CuentaFondoTipoFiltro>(new CuentaFondoTipoFiltro());
		init(false);
		setFocusGrid();
	}

	public WLCuentaFondoTipo(CuentaFondoTipoFiltro filtro) {
		super();		
		filterBI = new BeanItem<CuentaFondoTipoFiltro>(filtro);
		init(true);
		setFocusGrid();
	}
	
	protected CuentaFondoTipoService getService() throws Exception {
		if(service == null){
			service = AppCX.services().buildCuentaFondoTipoService();
		}
	...
	protected void buildContent() throws Exception {
		...
		confWinList(this, new CuentaFondoTipo().labelPlural());
	...
	protected void buildItemsGRDLayout() throws Exception {		
		...
		CuentaFondoTipo dto = new CuentaFondoTipo();
	...
	protected BeanItemContainer<CuentaFondoTipo> getItemsBIC() {
		...
		if (itemsBIC == null) {
			...
			itemsBIC = new BeanItemContainer<CuentaFondoTipo>(CuentaFondoTipo.class, new ArrayList<CuentaFondoTipo>());
		}
	...
	protected void addBeansToItemsBIC(boolean removeAllItems) {
		...
		try {
			...
			// List<CuentaFondoTipo> items = new CuentaFondoTipo().find(limit, offset, buildOrderBy(),
			// filterBI.getBean());
			...
			if (filterBI.getBean().equals(lastFilter) == false) {						
				...
				lastFilter = (CuentaFondoTipoFiltro) filterBI.getBean().clone();
				...
				List<CuentaFondoTipo> items = getService().find(filterBI.getBean());
				
				// Agrega los resultados a la grilla
				for (CuentaFondoTipo item : items) {
	...
	protected WindowForm buildWinddowForm(String mode, String id) throws Exception {
		return AppCX.widgets().buildWFCuentaFondoTipo(mode, id);
	}
...
// GENERATED BY ANTHILL 2019-06-28T11:36:36.585-03:00[America/Buenos_Aires]