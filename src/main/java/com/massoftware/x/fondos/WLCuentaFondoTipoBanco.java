...
public class WLCuentaFondoTipoBanco extends WindowListado {

	// -------------------------------------------------------------

	BeanItem<CuentaFondoTipoBancoFiltro> filterBI;
	protected BeanItemContainer<CuentaFondoTipoBanco> itemsBIC;
	...
	public WLCuentaFondoTipoBanco() {
		super();		
		filterBI = new BeanItem<CuentaFondoTipoBancoFiltro>(new CuentaFondoTipoBancoFiltro());
		init(false);
		setFocusGrid();
	}

	public WLCuentaFondoTipoBanco(CuentaFondoTipoBancoFiltro filtro) {
		super();		
		filterBI = new BeanItem<CuentaFondoTipoBancoFiltro>(filtro);
		init(true);
		setFocusGrid();
	}
	
	protected CuentaFondoTipoBancoService getService() throws Exception {
		if(service == null){
			service = AppCX.services().buildCuentaFondoTipoBancoService();
		}
	...
	protected void buildContent() throws Exception {
		...
		confWinList(this, new CuentaFondoTipoBanco().labelPlural());
	...
	protected void buildItemsGRDLayout() throws Exception {		
		...
		CuentaFondoTipoBanco dto = new CuentaFondoTipoBanco();
	...
	protected BeanItemContainer<CuentaFondoTipoBanco> getItemsBIC() {
		...
		if (itemsBIC == null) {
			...
			itemsBIC = new BeanItemContainer<CuentaFondoTipoBanco>(CuentaFondoTipoBanco.class, new ArrayList<CuentaFondoTipoBanco>());
		}
	...
	protected void addBeansToItemsBIC(boolean removeAllItems) {
		...
		try {
			...
			// List<CuentaFondoTipoBanco> items = new CuentaFondoTipoBanco().find(limit, offset, buildOrderBy(),
			// filterBI.getBean());
			...
			if (filterBI.getBean().equals(lastFilter) == false) {						
				...
				lastFilter = (CuentaFondoTipoBancoFiltro) filterBI.getBean().clone();
				...
				List<CuentaFondoTipoBanco> items = getService().find(filterBI.getBean());
				
				// Agrega los resultados a la grilla
				for (CuentaFondoTipoBanco item : items) {
	...
	protected WindowForm buildWinddowForm(String mode, String id) throws Exception {
		return AppCX.widgets().buildWFCuentaFondoTipoBanco(mode, id);
	}
...
// GENERATED BY ANTHILL 2019-06-27T10:39:13.016-03:00[America/Buenos_Aires]