...
public class WLCuentaFondoBancoCopia extends WindowListado {

	// -------------------------------------------------------------

	BeanItem<CuentaFondoBancoCopiaFiltro> filterBI;
	protected BeanItemContainer<CuentaFondoBancoCopia> itemsBIC;
	...
	public WLCuentaFondoBancoCopia() {
		super();		
		filterBI = new BeanItem<CuentaFondoBancoCopiaFiltro>(new CuentaFondoBancoCopiaFiltro());
		init(false);
		setFocusGrid();
	}

	public WLCuentaFondoBancoCopia(CuentaFondoBancoCopiaFiltro filtro) {
		super();		
		filterBI = new BeanItem<CuentaFondoBancoCopiaFiltro>(filtro);
		init(true);
		setFocusGrid();
	}
	
	protected CuentaFondoBancoCopiaService getService() throws Exception {
		if(service == null){
			service = AppCX.services().buildCuentaFondoBancoCopiaService();
		}
	...
	protected void buildContent() throws Exception {
		...
		confWinList(this, new CuentaFondoBancoCopia().labelPlural());
	...
	protected void buildItemsGRDLayout() throws Exception {		
		...
		CuentaFondoBancoCopia dto = new CuentaFondoBancoCopia();
	...
	protected BeanItemContainer<CuentaFondoBancoCopia> getItemsBIC() {
		...
		if (itemsBIC == null) {
			...
			itemsBIC = new BeanItemContainer<CuentaFondoBancoCopia>(CuentaFondoBancoCopia.class, new ArrayList<CuentaFondoBancoCopia>());
		}
	...
	protected void addBeansToItemsBIC(boolean removeAllItems) {
		...
		try {
			...
			// List<CuentaFondoBancoCopia> items = new CuentaFondoBancoCopia().find(limit, offset, buildOrderBy(),
			// filterBI.getBean());
			...
			if (filterBI.getBean().equals(lastFilter) == false) {						
				...
				lastFilter = (CuentaFondoBancoCopiaFiltro) filterBI.getBean().clone();
				...
				List<CuentaFondoBancoCopia> items = getService().find(filterBI.getBean());
				
				// Agrega los resultados a la grilla
				for (CuentaFondoBancoCopia item : items) {
	...
	protected WindowForm buildWinddowForm(String mode, String id) throws Exception {
		return AppCX.widgets().buildWFCuentaFondoBancoCopia(mode, id);
	}
...
// GENERATED BY ANTHILL 2019-06-28T11:36:36.648-03:00[America/Buenos_Aires]