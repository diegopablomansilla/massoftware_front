...
public class WLTalonarioControladorFizcal extends WindowListado {

	// -------------------------------------------------------------

	BeanItem<TalonarioControladorFizcalFiltro> filterBI;
	protected BeanItemContainer<TalonarioControladorFizcal> itemsBIC;
	...
	public WLTalonarioControladorFizcal() {
		super();		
		filterBI = new BeanItem<TalonarioControladorFizcalFiltro>(new TalonarioControladorFizcalFiltro());
		init(false);
		setFocusGrid();
	}

	public WLTalonarioControladorFizcal(TalonarioControladorFizcalFiltro filtro) {
		super();		
		filterBI = new BeanItem<TalonarioControladorFizcalFiltro>(filtro);
		init(true);
		setFocusGrid();
	}
	
	protected TalonarioControladorFizcalService getService() throws Exception {
		if(service == null){
			service = AppCX.services().buildTalonarioControladorFizcalService();
		}
	...
	protected void buildContent() throws Exception {
		...
		confWinList(this, new TalonarioControladorFizcal().labelPlural());
	...
	protected void buildItemsGRDLayout() throws Exception {		
		...
		TalonarioControladorFizcal dto = new TalonarioControladorFizcal();
	...
	protected BeanItemContainer<TalonarioControladorFizcal> getItemsBIC() {
		...
		if (itemsBIC == null) {
			...
			itemsBIC = new BeanItemContainer<TalonarioControladorFizcal>(TalonarioControladorFizcal.class, new ArrayList<TalonarioControladorFizcal>());
		}
	...
	protected void addBeansToItemsBIC(boolean removeAllItems) {
		...
		try {
			...
			// List<TalonarioControladorFizcal> items = new TalonarioControladorFizcal().find(limit, offset, buildOrderBy(),
			// filterBI.getBean());
			...
			if (filterBI.getBean().equals(lastFilter) == false) {						
				...
				lastFilter = (TalonarioControladorFizcalFiltro) filterBI.getBean().clone();
				...
				List<TalonarioControladorFizcal> items = getService().find(filterBI.getBean());
				
				// Agrega los resultados a la grilla
				for (TalonarioControladorFizcal item : items) {
	...
	protected WindowForm buildWinddowForm(String mode, String id) throws Exception {
		return AppCX.widgets().buildWFTalonarioControladorFizcal(mode, id);
	}
...
// GENERATED BY ANTHILL 2019-06-27T10:39:13.312-03:00[America/Buenos_Aires]