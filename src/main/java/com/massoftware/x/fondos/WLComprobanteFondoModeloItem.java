...
public class WLComprobanteFondoModeloItem extends WindowListado {

	// -------------------------------------------------------------

	BeanItem<ComprobanteFondoModeloItemFiltro> filterBI;
	protected BeanItemContainer<ComprobanteFondoModeloItem> itemsBIC;
	...
	protected ComboBoxBox cuentaFondoCBXB;
	protected SelectorBox cuentaFondoSBX;


	// -------------------------------------------------------------

	public WLComprobanteFondoModeloItem() {
		super();		
		filterBI = new BeanItem<ComprobanteFondoModeloItemFiltro>(new ComprobanteFondoModeloItemFiltro());
		init(false);
		setFocusGrid();
	}

	public WLComprobanteFondoModeloItem(ComprobanteFondoModeloItemFiltro filtro) {
		super();		
		filterBI = new BeanItem<ComprobanteFondoModeloItemFiltro>(filtro);
		init(true);
		setFocusGrid();
	}
	...
	protected void buildContent() throws Exception {
		...
		confWinList(this, new ComprobanteFondoModeloItem().labelPlural());
	...
	protected Component buildFiltros() throws Exception {		
		...
		if (cuentaFondoItems < MAX_ROWS_FOR_CBX) {
			...
			cuentaFondoCBXB = new ComboBoxBox(this, filterBI, "cuentaFondo", cuentaFondoLista, filterBI.getBean().getCuentaFondo());

		} else {

			cuentaFondoSBX = new SelectorBox(filterBI, "cuentaFondo") {
				...
				protected WindowListado getPopup(boolean filter) throws Exception {
					...
					//WLCuentaFondo windowPoPup = new WLCuentaFondo(filtro) {

						//protected void setSelectedItem() throws Exception {

							//cuentaFondoSBX.setSelectedItem(itemsGRD.getSelectedRow());

						//}

					//};

					WLCuentaFondo windowPoPup = AppCX.widgets().buildWLCuentaFondo(filtro);

					windowPoPup.selectorSBX = cuentaFondoSBX;
	...
	protected Component buildFiltrosLayout() throws Exception {	
		...
		if (cuentaFondoCBXB != null) {
			filaFiltroHL.addComponent(cuentaFondoCBXB);
		}
		if (cuentaFondoSBX != null) {
			filaFiltroHL.addComponent(cuentaFondoSBX);
		}
	...
	protected void buildItemsGRDLayout() throws Exception {		
		...
		itemsGRD.setColumns(new Object[] { "id", "numero", "debe", "comprobanteFondoModelo", "cuentaFondo" });
		...
		UtilUI.confColumn(itemsGRD.getColumn("debe"), true, 70);

		UtilUI.confColumn(itemsGRD.getColumn("comprobanteFondoModelo"), true, 240);

		UtilUI.confColumn(itemsGRD.getColumn("cuentaFondo"), true, -1);
		
		// ------------------------------------------------------------------

		ComprobanteFondoModeloItem dto = new ComprobanteFondoModeloItem();
		...
		itemsGRD.getColumn("debe").setRenderer(new HtmlRenderer(), new StringToBooleanConverter(FontAwesome.CHECK_SQUARE_O.getHtml(),FontAwesome.SQUARE_O.getHtml()));
	...
	protected BeanItemContainer<ComprobanteFondoModeloItem> getItemsBIC() {
		...
		if (itemsBIC == null) {
			...
			itemsBIC = new BeanItemContainer<ComprobanteFondoModeloItem>(ComprobanteFondoModeloItem.class, new ArrayList<ComprobanteFondoModeloItem>());
		}
	...
	protected void addBeansToItemsBIC(boolean removeAllItems) {
		...
		try {
			...
			// List<ComprobanteFondoModeloItem> items = new ComprobanteFondoModeloItem().find(limit, offset, buildOrderBy(),
			// filterBI.getBean());
			...
			if (filterBI.getBean().equals(lastFilter) == false) {						
				...
				lastFilter = (ComprobanteFondoModeloItemFiltro) filterBI.getBean().clone();
				...
				List<ComprobanteFondoModeloItem> items = getService().find(filterBI.getBean());
				
				// Agrega los resultados a la grilla
				for (ComprobanteFondoModeloItem item : items) {
	...
	protected WindowForm buildWinddowForm(String mode, String id) throws Exception {
		return AppCX.widgets().buildWFComprobanteFondoModeloItem(mode, id);
	}
...
// GENERATED BY ANTHILL 2019-06-28T11:36:36.902-03:00[America/Buenos_Aires]