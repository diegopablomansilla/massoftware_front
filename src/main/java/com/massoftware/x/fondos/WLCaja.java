...
public class WLCaja extends WindowListado {

	// -------------------------------------------------------------

	BeanItem<CajaFiltro> filterBI;
	...
	public WLCaja() {
		super();		
		filterBI = new BeanItem<CajaFiltro>(new CajaFiltro());
		init(false);
		setFocusGrid();
	}

	public WLCaja(CajaFiltro filtro) {
		super();		
		filterBI = new BeanItem<CajaFiltro>(filtro);
		init(true);
		setFocusGrid();
	}
	...
	protected void buildItemsGRDLayout() throws Exception {		
		...
		itemsGRD.setColumns(new Object[] { "id", "numero", "nombre", "seguridadPuerta" });
	...
	protected void addBeansToItemsBIC(boolean removeAllItems) {
		...
		try {
			...
			// List<Caja> items = new Caja().find(limit, offset, buildOrderBy(),
			// filterBI.getBean());
			...
			if (filterBI.getBean().equals(lastFilter) == false) {						
				...
				lastFilter = (CajaFiltro) filterBI.getBean().clone();
				...
				List<Caja> items = getService().find(filterBI.getBean());
	...
	protected WindowForm buildWinddowForm(String mode, String id) throws Exception {
		return AppCX.widgets().buildWFCaja(mode, id);
	}
...
// GENERATED BY ANTHILL 2019-06-27T10:39:12.939-03:00[America/Buenos_Aires]