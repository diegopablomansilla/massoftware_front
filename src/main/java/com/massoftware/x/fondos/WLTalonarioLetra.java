...
public class WLTalonarioLetra extends WindowListado {

	// -------------------------------------------------------------

	BeanItem<TalonarioLetraFiltro> filterBI;
	protected BeanItemContainer<TalonarioLetra> itemsBIC;
	...
	public WLTalonarioLetra() {
		super();		
		filterBI = new BeanItem<TalonarioLetraFiltro>(new TalonarioLetraFiltro());
		init(false);
		setFocusGrid();
	}

	public WLTalonarioLetra(TalonarioLetraFiltro filtro) {
		super();		
		filterBI = new BeanItem<TalonarioLetraFiltro>(filtro);
		init(true);
		setFocusGrid();
	}
	
	protected TalonarioLetraService getService() throws Exception {
		if(service == null){
			service = AppCX.services().buildTalonarioLetraService();
		}
	...
	protected void buildContent() throws Exception {
		...
		confWinList(this, new TalonarioLetra().labelPlural());
	...
	protected void buildItemsGRDLayout() throws Exception {		
		...
		itemsGRD.setColumns(new Object[] { "id", "nombre" });
		...
		TalonarioLetra dto = new TalonarioLetra();
		...
		order.add(new SortOrder("nombre", SortDirection.DESCENDING));
	...
	protected BeanItemContainer<TalonarioLetra> getItemsBIC() {
		...
		if (itemsBIC == null) {
			...
			itemsBIC = new BeanItemContainer<TalonarioLetra>(TalonarioLetra.class, new ArrayList<TalonarioLetra>());
		}
	...
	protected void addBeansToItemsBIC(boolean removeAllItems) {
		...
		try {
			...
			// List<TalonarioLetra> items = new TalonarioLetra().find(limit, offset, buildOrderBy(),
			// filterBI.getBean());
			...
			if (filterBI.getBean().equals(lastFilter) == false) {						
				...
				lastFilter = (TalonarioLetraFiltro) filterBI.getBean().clone();
				...
				List<TalonarioLetra> items = getService().find(filterBI.getBean());
				
				// Agrega los resultados a la grilla
				for (TalonarioLetra item : items) {
	...
	protected WindowForm buildWinddowForm(String mode, String id) throws Exception {
		return AppCX.widgets().buildWFTalonarioLetra(mode, id);
	}
...
// GENERATED BY ANTHILL 2019-06-29T09:32:09.711-03:00[America/Buenos_Aires]