...
public class WLTipoComprobanteCopiaAlternativo extends WindowListado {

	// -------------------------------------------------------------

	BeanItem<TipoComprobanteCopiaAlternativoFiltro> filterBI;
	protected BeanItemContainer<TipoComprobanteCopiaAlternativo> itemsBIC;
	...
	public WLTipoComprobanteCopiaAlternativo() {
		super();		
		filterBI = new BeanItem<TipoComprobanteCopiaAlternativoFiltro>(new TipoComprobanteCopiaAlternativoFiltro());
		init(false);
		setFocusGrid();
	}

	public WLTipoComprobanteCopiaAlternativo(TipoComprobanteCopiaAlternativoFiltro filtro) {
		super();		
		filterBI = new BeanItem<TipoComprobanteCopiaAlternativoFiltro>(filtro);
		init(true);
		setFocusGrid();
	}
	
	protected TipoComprobanteCopiaAlternativoService getService() throws Exception {
		if(service == null){
			service = AppCX.services().buildTipoComprobanteCopiaAlternativoService();
		}
	...
	protected void buildContent() throws Exception {
		...
		confWinList(this, new TipoComprobanteCopiaAlternativo().labelPlural());
	...
	protected void buildItemsGRDLayout() throws Exception {		
		...
		TipoComprobanteCopiaAlternativo dto = new TipoComprobanteCopiaAlternativo();
	...
	protected BeanItemContainer<TipoComprobanteCopiaAlternativo> getItemsBIC() {
		...
		if (itemsBIC == null) {
			...
			itemsBIC = new BeanItemContainer<TipoComprobanteCopiaAlternativo>(TipoComprobanteCopiaAlternativo.class, new ArrayList<TipoComprobanteCopiaAlternativo>());
		}
	...
	protected void addBeansToItemsBIC(boolean removeAllItems) {
		...
		try {
			...
			// List<TipoComprobanteCopiaAlternativo> items = new TipoComprobanteCopiaAlternativo().find(limit, offset, buildOrderBy(),
			// filterBI.getBean());
			...
			if (filterBI.getBean().equals(lastFilter) == false) {						
				...
				lastFilter = (TipoComprobanteCopiaAlternativoFiltro) filterBI.getBean().clone();
				...
				List<TipoComprobanteCopiaAlternativo> items = getService().find(filterBI.getBean());
				
				// Agrega los resultados a la grilla
				for (TipoComprobanteCopiaAlternativo item : items) {
	...
	protected WindowForm buildWinddowForm(String mode, String id) throws Exception {
		return AppCX.widgets().buildWFTipoComprobanteCopiaAlternativo(mode, id);
	}
...
// GENERATED BY ANTHILL 2019-06-29T09:32:10.380-03:00[America/Buenos_Aires]