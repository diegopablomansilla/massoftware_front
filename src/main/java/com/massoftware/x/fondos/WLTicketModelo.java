...
public class WLTicketModelo extends WindowListado {

	// -------------------------------------------------------------

	BeanItem<TicketModeloFiltro> filterBI;
	protected BeanItemContainer<TicketModelo> itemsBIC;
	...
	protected ComboBoxBox ticketCBXB;
	protected SelectorBox ticketSBX;


	// -------------------------------------------------------------

	public WLTicketModelo() {
		super();		
		filterBI = new BeanItem<TicketModeloFiltro>(new TicketModeloFiltro());
		init(false);
		setFocusGrid();
	}

	public WLTicketModelo(TicketModeloFiltro filtro) {
		super();		
		filterBI = new BeanItem<TicketModeloFiltro>(filtro);
		init(true);
		setFocusGrid();
	}
	...
	protected void buildContent() throws Exception {
		...
		confWinList(this, new TicketModelo().labelPlural());
	...
	protected Component buildFiltros() throws Exception {		
		...
		if (ticketItems < MAX_ROWS_FOR_CBX) {
			...
			ticketCBXB = new ComboBoxBox(this, filterBI, "ticket", ticketLista, filterBI.getBean().getTicket());

		} else {

			ticketSBX = new SelectorBox(filterBI, "ticket") {
				...
				protected WindowListado getPopup(boolean filter) throws Exception {
					...
					//WLTicket windowPoPup = new WLTicket(filtro) {

						//protected void setSelectedItem() throws Exception {

							//ticketSBX.setSelectedItem(itemsGRD.getSelectedRow());

						//}

					//};

					WLTicket windowPoPup = AppCX.widgets().buildWLTicket(filtro);

					windowPoPup.selectorSBX = ticketSBX;
	...
	protected Component buildFiltrosLayout() throws Exception {	
		...
		if (ticketCBXB != null) {
			filaFiltroHL.addComponent(ticketCBXB);
		}
		if (ticketSBX != null) {
			filaFiltroHL.addComponent(ticketSBX);
		}
	...
	protected void buildItemsGRDLayout() throws Exception {		
		...
		itemsGRD.setColumns(new Object[] { "id", "numero", "nombre", "ticket", "pruebaLectura", "activo", "longitudLectura", "identificacionPosicion", "identificacion", "importePosicion", "longitud", "cantidadDecimales", "numeroPosicion", "numeroLongitud", "prefijoIdentificacion", "posicionPrefijo" });
		...
		UtilUI.confColumn(itemsGRD.getColumn("ticket"), true, 240);

		UtilUI.confColumn(itemsGRD.getColumn("pruebaLectura"), true, 240);

		UtilUI.confColumn(itemsGRD.getColumn("activo"), true, 70);

		UtilUI.confColumn(itemsGRD.getColumn("longitudLectura"), true, 100);

		UtilUI.confColumn(itemsGRD.getColumn("identificacionPosicion"), true, 100);

		UtilUI.confColumn(itemsGRD.getColumn("identificacion"), true, 100);

		UtilUI.confColumn(itemsGRD.getColumn("importePosicion"), true, 100);

		UtilUI.confColumn(itemsGRD.getColumn("longitud"), true, 100);

		UtilUI.confColumn(itemsGRD.getColumn("cantidadDecimales"), true, 100);

		UtilUI.confColumn(itemsGRD.getColumn("numeroPosicion"), true, 100);

		UtilUI.confColumn(itemsGRD.getColumn("numeroLongitud"), true, 100);

		UtilUI.confColumn(itemsGRD.getColumn("prefijoIdentificacion"), true, 240);

		UtilUI.confColumn(itemsGRD.getColumn("posicionPrefijo"), true, -1);
		
		// ------------------------------------------------------------------

		TicketModelo dto = new TicketModelo();
		...
		itemsGRD.getColumn("activo").setRenderer(new HtmlRenderer(), new StringToBooleanConverter(FontAwesome.CHECK_SQUARE_O.getHtml(),FontAwesome.SQUARE_O.getHtml()));
	...
	protected BeanItemContainer<TicketModelo> getItemsBIC() {
		...
		if (itemsBIC == null) {
			...
			itemsBIC = new BeanItemContainer<TicketModelo>(TicketModelo.class, new ArrayList<TicketModelo>());
		}
	...
	protected void addBeansToItemsBIC(boolean removeAllItems) {
		...
		try {
			...
			// List<TicketModelo> items = new TicketModelo().find(limit, offset, buildOrderBy(),
			// filterBI.getBean());
			...
			if (filterBI.getBean().equals(lastFilter) == false) {						
				...
				lastFilter = (TicketModeloFiltro) filterBI.getBean().clone();
				...
				List<TicketModelo> items = getService().find(filterBI.getBean());
				
				// Agrega los resultados a la grilla
				for (TicketModelo item : items) {
	...
	protected WindowForm buildWinddowForm(String mode, String id) throws Exception {
		return AppCX.widgets().buildWFTicketModelo(mode, id);
	}
...
// GENERATED BY ANTHILL 2019-07-08T12:47:41.296-03:00[America/Buenos_Aires]