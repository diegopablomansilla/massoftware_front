...
public class WLSucursal extends WindowListado {

	// -------------------------------------------------------------

	BeanItem<SucursalFiltro> filterBI;
	...
	public WLSucursal(SucursalFiltro filtro) {
		super();		
		filterBI = new BeanItem<SucursalFiltro>(filtro);
		init(true);
		setFocusGrid();
	}
	...
	protected void buildItemsGRDLayout() throws Exception {		
		...
		UtilUI.confColumn(itemsGRD.getColumn("tipoSucursal"), true, 240);

		UtilUI.confColumn(itemsGRD.getColumn("cuentaClientesDesde"), true, 72);

		UtilUI.confColumn(itemsGRD.getColumn("cuentaClientesHasta"), true, 72);

		UtilUI.confColumn(itemsGRD.getColumn("cantidadCaracteresClientes"), true, 100);

		UtilUI.confColumn(itemsGRD.getColumn("identificacionNumericaClientes"), true, 70);

		UtilUI.confColumn(itemsGRD.getColumn("permiteCambiarClientes"), true, 70);

		UtilUI.confColumn(itemsGRD.getColumn("cuentaProveedoresDesde"), true, 84);

		UtilUI.confColumn(itemsGRD.getColumn("cuentaProveedoresHasta"), true, 84);

		UtilUI.confColumn(itemsGRD.getColumn("cantidadCaracteresProveedores"), true, 100);

		UtilUI.confColumn(itemsGRD.getColumn("identificacionNumericaProveedores"), true, 70);

		UtilUI.confColumn(itemsGRD.getColumn("permiteCambiarProveedores"), true, 70);

		UtilUI.confColumn(itemsGRD.getColumn("clientesOcacionalesDesde"), true, 100);

		UtilUI.confColumn(itemsGRD.getColumn("clientesOcacionalesHasta"), true, 100);

		UtilUI.confColumn(itemsGRD.getColumn("numeroCobranzaDesde"), true, 100);

		UtilUI.confColumn(itemsGRD.getColumn("numeroCobranzaHasta"), true, -1);
		...
		itemsGRD.getColumn("identificacionNumericaClientes").setRenderer(new HtmlRenderer(), new StringToBooleanConverter(FontAwesome.CHECK_SQUARE_O.getHtml(),FontAwesome.SQUARE_O.getHtml()));

		itemsGRD.getColumn("permiteCambiarClientes").setRenderer(new HtmlRenderer(), new StringToBooleanConverter(FontAwesome.CHECK_SQUARE_O.getHtml(),FontAwesome.SQUARE_O.getHtml()));

		itemsGRD.getColumn("identificacionNumericaProveedores").setRenderer(new HtmlRenderer(), new StringToBooleanConverter(FontAwesome.CHECK_SQUARE_O.getHtml(),FontAwesome.SQUARE_O.getHtml()));

		itemsGRD.getColumn("permiteCambiarProveedores").setRenderer(new HtmlRenderer(), new StringToBooleanConverter(FontAwesome.CHECK_SQUARE_O.getHtml(),FontAwesome.SQUARE_O.getHtml()));
	...
	protected void addBeansToItemsBIC(boolean removeAllItems) {
		...
		try {
			...
			// List<Sucursal> items = new Sucursal().find(limit, offset, buildOrderBy(),
			// filterBI.getBean());
			...
			if (filterBI.getBean().equals(lastFilter) == false) {						
				...
				lastFilter = (SucursalFiltro) filterBI.getBean().clone();
				...
				List<Sucursal> items = getService().find(filterBI.getBean());
	...
	protected WindowForm buildWinddowForm(String mode, String id) throws Exception {
		return AppCX.widgets().buildWFSucursal(mode, id);
	}
...
// GENERATED BY ANTHILL 2019-06-17T20:59:20.651-03:00[America/Buenos_Aires]