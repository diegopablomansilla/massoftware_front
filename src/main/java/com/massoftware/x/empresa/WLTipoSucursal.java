...
public class WLTipoSucursal extends WindowListado {

	// -------------------------------------------------------------

	BeanItem<TipoSucursalFiltro> filterBI;
	protected BeanItemContainer<TipoSucursal> itemsBIC;
	...
	public WLTipoSucursal() {
		super();		
		filterBI = new BeanItem<TipoSucursalFiltro>(new TipoSucursalFiltro());
		init(false);
		setFocusGrid();
	}

	public WLTipoSucursal(TipoSucursalFiltro filtro) {
		super();		
		filterBI = new BeanItem<TipoSucursalFiltro>(filtro);
		init(true);
		setFocusGrid();
	}
	
	protected TipoSucursalService getService() throws Exception {
		if(service == null){
			service = AppCX.services().buildTipoSucursalService();
		}
	...
	protected void buildContent() throws Exception {
		...
		confWinList(this, new TipoSucursal().labelPlural());
	...
	protected void buildItemsGRDLayout() throws Exception {		
		...
		TipoSucursal dto = new TipoSucursal();
	...
	protected BeanItemContainer<TipoSucursal> getItemsBIC() {
		...
		if (itemsBIC == null) {
			...
			itemsBIC = new BeanItemContainer<TipoSucursal>(TipoSucursal.class, new ArrayList<TipoSucursal>());
		}
	...
	protected void addBeansToItemsBIC(boolean removeAllItems) {
		...
		try {
			...
			// List<TipoSucursal> items = new TipoSucursal().find(limit, offset, buildOrderBy(),
			// filterBI.getBean());
			...
			if (filterBI.getBean().equals(lastFilter) == false) {						
				...
				lastFilter = (TipoSucursalFiltro) filterBI.getBean().clone();
				...
				List<TipoSucursal> items = getService().find(filterBI.getBean());
				
				// Agrega los resultados a la grilla
				for (TipoSucursal item : items) {
	...
	protected WindowForm buildWinddowForm(String mode, String id) throws Exception {
		return AppCX.widgets().buildWFTipoSucursal(mode, id);
	}
...
// GENERATED BY ANTHILL 2019-06-17T20:59:20.619-03:00[America/Buenos_Aires]