...
public class WLDepositoModulo extends WindowListado {

	// -------------------------------------------------------------

	BeanItem<DepositoModuloFiltro> filterBI;
	protected BeanItemContainer<DepositoModulo> itemsBIC;
	...
	public WLDepositoModulo() {
		super();		
		filterBI = new BeanItem<DepositoModuloFiltro>(new DepositoModuloFiltro());
		init(false);
		setFocusGrid();
	}

	public WLDepositoModulo(DepositoModuloFiltro filtro) {
		super();		
		filterBI = new BeanItem<DepositoModuloFiltro>(filtro);
		init(true);
		setFocusGrid();
	}
	
	protected DepositoModuloService getService() throws Exception {
		if(service == null){
			service = AppCX.services().buildDepositoModuloService();
		}
	...
	protected void buildContent() throws Exception {
		...
		confWinList(this, new DepositoModulo().labelPlural());
	...
	protected void buildItemsGRDLayout() throws Exception {		
		...
		DepositoModulo dto = new DepositoModulo();
	...
	protected BeanItemContainer<DepositoModulo> getItemsBIC() {
		...
		if (itemsBIC == null) {
			...
			itemsBIC = new BeanItemContainer<DepositoModulo>(DepositoModulo.class, new ArrayList<DepositoModulo>());
		}
	...
	protected void addBeansToItemsBIC(boolean removeAllItems) {
		...
		try {
			...
			// List<DepositoModulo> items = new DepositoModulo().find(limit, offset, buildOrderBy(),
			// filterBI.getBean());
			...
			if (filterBI.getBean().equals(lastFilter) == false) {						
				...
				lastFilter = (DepositoModuloFiltro) filterBI.getBean().clone();
				...
				List<DepositoModulo> items = getService().find(filterBI.getBean());
				
				// Agrega los resultados a la grilla
				for (DepositoModulo item : items) {
	...
	protected WindowForm buildWinddowForm(String mode, String id) throws Exception {
		return AppCX.widgets().buildWFDepositoModulo(mode, id);
	}
...
// GENERATED BY ANTHILL 2019-06-28T11:36:35.931-03:00[America/Buenos_Aires]