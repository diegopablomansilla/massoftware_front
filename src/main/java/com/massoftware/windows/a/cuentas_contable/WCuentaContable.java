...
import com.massoftware.x.util.controls.OptionGroupEntity;
...
public class WCuentaContable extends WindowForm {
	...
	private TabSheet buildCuerpo() throws Exception {
		...
		this.addShortcutListener(new ShortcutListener("ENTER", KeyCode.ENTER, new int[] {}) {
			...
			public void handleAction(Object sender, Object target) {
				...
				try {
					...
					if (target instanceof TextField && ((TextField) target).getParent().equals(seguridadPuertaSB)) {
						seguridadPuertaSB.blur();
					}
		...
		cuentaJerarquiaTXT
				.addValidator(new CuentaJerarquiaValidator(integraTXT, String.class, "cuentaJerarquia", this.itemBI));
		...
		// ---------------------------------------------------------------------------------------------------------
		seguridadPuertaSB = new WCPuertaSB(this.itemBI);
		// ---------------------------------------------------------------------------------------------------------
		...
		tabSheet = UtilUI.buildTS();

		tabSheet.addTab(generalHL, "General");
		centroCostoTab = tabSheet.addTab(centroCostoVL, "Centro de costo");
		tabSheet.addTab(seguridadVL, "Seguridad");

		// ---------------------------------------------------------------------------------------------------------

		imputableCHK.addValueChangeListener(listener -> imputableCKBChange());
	...
	private void imputableCKBChange() {
		try {
			...
			boolean b = itemBI.getBean().getImputable().booleanValue();

			// pestaniaCCVL.setEnabled(b);
			centroCostoTab.setEnabled(b);
